...
CREATE TABLE public.receitas (
    ...
    CONSTRAINT receitas_dificuldade_check CHECK (((dificuldade)::text = ANY (ARRAY[('Fácil'::character varying)::text, ('Média'::character varying)::text, ('Difícil'::character varying)::text])))
);
...
COPY public.receitas (id, titulo, descricao, modo_preparo, imagem, dificuldade, tempo_preparo) FROM stdin;
13	Sopa	Legumes cozidos	Teste	\N	Fácil	40
17	Morcego cozido	morcego com batatas	Teste	\N	Média	50
19	Ensopado de Cérebro Mutante	Uma receita tradicional dos sobreviventes do apocalipse zumbi, com um toque crocante.	Teste	\N	Média	45
\.
...
SELECT pg_catalog.setval('public.receitas_id_seq', 20, true);
